  ...
  return (
    <article className="limite-leitura empilha">
      ...
      <form className="cartao cartao__corpo empilha" onSubmit={async (e) => {
        e.preventDefault(); definirMensagem('Entrando…')
        try {
        ...
        <label className="campo"><span className="campo__rotulo">Senha</span><input className="entrada" type="password" required autoComplete="current-password" value={senha} onChange={(e) => definirSenha(e.target.value)} /></label>
        <button className="botao botao--principal" type="submit" disabled={!configurado}>Entrar com senha</button>
        <button className="botao" type="button" disabled={!configurado} onClick={async () => {
          definirMensagem('Enviando…')
          try {
            await enviarLinkAcesso(email)
            definirMensagem('Confira seu e-mail e abra o link de acesso neste navegador.')
          } catch (erro) {
            definirMensagem(erro instanceof Error ? erro.message : 'Não foi possível entrar.')
          }
        }}>Enviar link mágico</button>
