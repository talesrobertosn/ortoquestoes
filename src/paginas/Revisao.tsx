 ...
 */
const ESCADAS = [
  { rotulo: 'Foi um chute', dias: [1, 3, 7, 14], nota: 'Volta já amanhã: acertar sem saber por quê não fixa nada.' },
  { rotulo: 'Tinha dúvida', dias: [2, 5, 10, 21], nota: 'Volta antes do ciclo normal, para firmar o raciocínio.' },
  { rotulo: 'Tinha certeza', dias: [3, 7, 14, 30], nota: 'Ciclo completo: o intervalo cresce porque o conceito está firme.' },
] as const
...
export function Revisao() {
  ...
  return <div className="empilha-2 revisao-premium">
    ...
    <section className="cartao cartao__corpo"><p className="meta">COMO A FILA É MONTADA</p><h2>Chute e dúvida voltam mais cedo</h2>
      ...
      <div className="escada">{ESCADAS.map(e => <div key={e.rotulo} className="escada__linha">
        <div className="escada__topo"><strong>{e.rotulo}</strong><span className="escada__degraus">{e.dias.map(d => <em key={d}>{d}d</em>)}</span></div>
        <small>{e.nota}</small>
      </div>)}</div>
      ...
      {incertas > 0 && <div className="linha"><a className="botao" href="#/treinar?situacao=incertas">Refazer as {incertas} que respondi com dúvida ou chute</a></div>}
    </section>
